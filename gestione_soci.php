...
// Normalizza: alcune chiamate al controller possono ritornare `false` in caso di errore.
// Evitiamo passare `false` a `count()` o `foreach` convertendo in array vuoto.
if (!is_array($soci)) {
    $soci = [];
}
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="stat-value mb-0"><?php echo ($stats['percentuale'] ?? $stats['percentuale_attivi'] ?? 0); ?>%</h3>
                        </div>
                        <div class="stat-icon">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" onclick="salvaSocio()">
                    <i class="bi bi-check-circle"></i> Salva Socio
                </button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="editSocioModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title">
                    <i class="bi bi-pencil"></i> Modifica Socio
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="formModificaSocio">
                    <input type="hidden" name="id" id="editSocioId">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Nome *</label>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" onclick="aggiornaSocio()">
...
<!-- Modal Visualizza Socio -->
<div class="modal fade" id="viewSocioModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="background: linear-gradient(135deg, #ff6b35 0%, #ff8c5a 100%); color: white;">
                <h5 class="modal-title">
                    <i class="bi bi-person-circle"></i> <span id="viewSocioNome"></span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" id="viewSocioBody">
...
<div class="modal fade" id="confermaDisattivazioneModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Sei sicuro di voler disattivare l'socio <strong id="nomeSocioDaDisattivare"></strong>?</p>
                <div class="alert alert-warning">
                    <i class="bi bi-info-circle"></i>
                    L'socio non verrà eliminato ma solo disattivato. Potrà essere riattivato in seguito.
                </div>
                <input type="hidden" id="idSocioDaDisattivare">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                    <i class="bi bi-x-circle"></i> Annulla
                </button>
                <button type="button" class="btn btn-danger" onclick="disattivaSocio()">
...
// Carica info lezioni per ogni socio all'avvio
...
function filtroTabella() {
    ...
    document.getElementById('countSoci').textContent = visibili;
}

function resetFiltri() {
    document.getElementById('searchSoci').value = '';
    document.getElementById('filtroLezioni').value = 'tutti';
    filtroTabella();
}

function salvaSocio() {
    const form = document.getElementById('formNuovoSocio');
    ...
    .then(result => {
        if (result.success) {
            mostraToast('Successo', 'Socio creato correttamente', 'success');
